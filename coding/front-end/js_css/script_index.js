  ...
  if (promptSearchButton) {
    promptSearchButton.addEventListener('click', function(event) {
      ...
      makeAPIRequest(searchValue, function(response) {
        ...
        window.location.href = `prompt-search-result.html?response=${encodedResponse}`;  // for local
        // window.location.href = `prompt-search-result?response=${encodedResponse}`;  // for public
      });
    });
  }
});